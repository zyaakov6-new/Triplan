...
const T = {
  he: {
    ...
    start: 'מתחילים!',
    slides: [
      { title: 'כל הטיול שלכם, על מפה',    subtitle: 'עצירות, מסלולים ומרחקים — הכל בצמוד לתכנית' },
      { title: 'תכנן עם כל הקבוצה',         subtitle: 'לינק אחד — כל שינוי מתעדכן לכולם בזמן אמת' },
      { title: 'הכל במקום אחד',             subtitle: 'ציוד, תקציב, עצירות ותמונות — בלי גוגל דוקס ובלי וואטסאפ' },
    ],
    welcomeBack:    'ברוכים הבאים חזרה',
    readyToGo:      'מוכנים לצאת לדרך?',
    signInSub:      'כנסו לחשבון שלכם',
    signUpSub:      'צרו חשבון בחינם — לוקח 10 שניות',
  ...
  en: {
    ...
    start: "Let's go!",
    slides: [
      { title: 'Your whole trip, on a map',       subtitle: 'Stops, routes and distances — all alongside your plan' },
      { title: 'Plan with your whole group',       subtitle: 'One link — every change syncs for everyone in real time' },
      { title: 'Everything in one place',          subtitle: 'Gear, budget, stops and photos — no Google Docs, no WhatsApp' },
    ],
    welcomeBack:    'Welcome back',
    readyToGo:      'Ready to hit the road?',
    signInSub:      'Sign in to your account',
    signUpSub:      'Create a free account — takes 10 seconds',
...
// Detect language once per session, cache in localStorage for 24 h
function detectLang(setLang) {
  ...
  // Instant fallback from browser locale while IP resolves
  ...
  fetch('https://ipapi.co/json/')
    .then(r => r.json())
    .then(({ country_code }) => {
      const lang = country_code === 'IL' ? 'he' : 'en'
      localStorage.setItem(KEY, lang)
      localStorage.setItem(TS, String(Date.now()))
      setLang(lang)
    })
    .catch(() => {
      // Keep browser-locale fallback; don't persist so we retry next visit
    })
}

// ── Slide illustrations ───────────────────────────────────────────────────────

function MapIllustration({ isHe }) {
  return (
    <svg viewBox="0 0 320 240" style={{ width: '100%', height: '100%' }}>
      ...
      {isHe ? (
        ...
        <>
          <text x="160" y="70" textAnchor="middle" fill="rgba(255,255,255,0.7)" fontSize="11" fontFamily="monospace">154 mi</text>
          <text x="50"  y="208" textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize="10">Paris</text>
          <text x="165" y="138" textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize="10">Lyon</text>
          <text x="270" y="168" textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize="10">Nice</text>
        </>
      )}
    </svg>
  )
...
function CollabIllustration({ isHe }) {
  return (
    <svg viewBox="0 0 320 240" style={{ width: '100%', height: '100%' }}>
      ...
      <text x="80"  y="66"  textAnchor="middle" fill="white" fontSize="15" fontWeight="700">{isHe ? 'א' : 'A'}</text>
      ...
      <text x="240" y="66"  textAnchor="middle" fill="white" fontSize="15" fontWeight="700">{isHe ? 'ב' : 'B'}</text>
      ...
      <text x="160" y="201" textAnchor="middle" fill="white" fontSize="15" fontWeight="700">{isHe ? 'ג' : 'C'}</text>
      <rect x="112" y="14" width="96" height="22" rx="11" fill="rgba(45,158,110,0.25)" stroke="#2D9E6E" strokeWidth="1"/>
      <circle cx="128" cy="25" r="4" fill="#2D9E6E"/>
      <text x="152" y="29" fill="#2D9E6E" fontSize="11" fontWeight="600">{isHe ? 'בזמן אמת' : 'Live sync'}</text>
    </svg>
  )
}

function AllInOneIllustration({ isHe }) {
  const stops = isHe
    ? [
        { y: 94,  color: '#C4622D', done: true  },
        { y: 122, color: '#2D9E6E', done: true  },
        { y: 150, color: '#5B8FD4', done: false },
        { y: 178, color: '#E8A020', done: false },
      ]
    : [
        { y: 94,  color: '#C4622D', done: true  },
        { y: 122, color: '#2D9E6E', done: true  },
  ...
  return (
    <svg viewBox="0 0 320 240" style={{ width: '100%', height: '100%' }}>
      ...
      <text x="160" y="48" textAnchor="middle" fill="rgba(255,255,255,0.9)" fontSize="13" fontWeight="600">
        {isHe ? 'טיול לאיטליה 🇮🇹' : 'Italy Trip 🇮🇹'}
      </text>
      <text x="160" y="63" textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize="10">
      ...
      {stops.map(({ y, color, done }) => (
        <g key={y}>
          ...
          <text x="254" y={y + 11} textAnchor="middle" fill={color} fontSize="9" fontWeight="600">
            {isHe ? '₪180' : '$50'}
          </text>
        </g>
      ))}
        ...
        <div style={{ position: 'relative' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
            <button onClick={onBack} style={{ width: 34, height: 34, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
              <Icon name={isRtl ? 'chevron_right' : 'chevron_left'} size={16} color="rgba(255,255,255,0.6)" />
            </button>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 30, color: 'white', fontWeight: 400, marginBottom: 6, lineHeight: 1.2 }}>
            {mode === 'login' ? t.welcomeBack : t.readyToGo}
          </h2>
...
export default function AuthPage() {
  const [lang, setLang] = useState(() => {
    ...
    const ts     = parseInt(localStorage.getItem('triplan_lang_ts') || '0', 10)
  ...
  const SLIDES = [
    { bg: 'linear-gradient(160deg, #1A1612 0%, #2D1A0E 70%, #3A200E 100%)', illustration: <MapIllustration isHe={isHe}/>,      accent: '#C4622D', ...t.slides[0] },
    { bg: 'linear-gradient(160deg, #0D1F1F 0%, #0D2B2B 70%, #102F2F 100%)', illustration: <CollabIllustration isHe={isHe}/>,   accent: '#2D9E6E', ...t.slides[1] },
    { bg: 'linear-gradient(160deg, #0F1A2E 0%, #152340 70%, #182B50 100%)', illustration: <AllInOneIllustration isHe={isHe}/>, accent: '#5B8FD4', ...t.slides[2] },
  ]

  const [view, setView]   = useState(() => localStorage.getItem('triplan_onboarded') ? 'auth' : 'slides')
  const [slide, setSlide] = useState(0)
  const touchStartX       = useRef(0)

  const finish = () => { localStorage.setItem('triplan_onboarded', '1'); setView('auth') }
  const next   = () => { if (slide < SLIDES.length - 1) setSlide(s => s + 1); else finish() }
  ...
  if (view === 'auth') return <AuthForm onBack={() => setView('slides')} t={t} />

  const current = SLIDES[slide]
  const isLast  = slide === SLIDES.length - 1

  // Skip button: top-left for LTR, top-left for RTL (opposite reading end)
  const skipStyle = isRtl
    ? { position: 'absolute', top: 'calc(var(--safe-top) + 16px)', left: 20, zIndex: 10 }
    : { position: 'absolute', top: 'calc(var(--safe-top) + 16px)', right: 20, zIndex: 10 }
  const logoStyle = isRtl
    ? { position: 'absolute', top: 'calc(var(--safe-top) + 16px)', right: 20, zIndex: 10 }
    : { position: 'absolute', top: 'calc(var(--safe-top) + 16px)', left: 20,  zIndex: 10 }

  return (
    <div dir={t.dir} style={{ height: '100%', display: 'flex', flexDirection: 'column', background: current.bg, transition: 'background 0.5s ease', overflow: 'hidden' }}
      ...
      <div style={{ background: 'rgba(255,255,255,0.05)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(255,255,255,0.1)', borderRadius: '28px 28px 0 0', padding: '28px 28px calc(var(--safe-bottom) + 28px)', flexShrink: 0 }}>
        ...
        <h2 key={`title-${slide}`} style={{ fontFamily: 'var(--font-display)', fontSize: 26, color: 'white', fontWeight: 400, marginBottom: 10, lineHeight: 1.3, textAlign: isRtl ? 'right' : 'left', animation: 'slideUp 0.35s ease' }}>
          {current.title}
        </h2>
        <p key={`sub-${slide}`} style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14, lineHeight: 1.6, marginBottom: 24, textAlign: isRtl ? 'right' : 'left', animation: 'slideUp 0.4s ease' }}>
          {current.subtitle}
        </p>
